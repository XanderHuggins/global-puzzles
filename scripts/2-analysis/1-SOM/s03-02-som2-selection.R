...
# # cutoff.point <- findCutoff(knee_df$wss, knee_df$kclust, method="first", 0.1)
# # cutoff.point <- findCutoff(knee_df$wss, knee_df$kclust, method="first", 0.1)
# # knee_deriv <- KneeArrower::derivative(knee_df$wss, knee_df$kclust, m = 1)
# # knee_deriv(1)
# # best_nc = 6 # visible from curve
# 
...
# 
# top = norm_extract |> dplyr::filter(as.numeric(x) == as.numeric(best_nc)[1]) |> pull(y)
...
# import the size bias function


##
### ------------------- Stage 0: create scaled metrics \\
##
size_bias = read_rds(here("data/som2_custom_size_bias.rds"))
...
# size bias
df = merge(x = df, y = size_bias, by.x = "som_size", by.y = "som_size")
plot(df$bias ~ df$som_size)
...
for (jj in 1:nrow(sensitivity_df)) {
  ...
  df_keep_combined$perf_x_size = minmaxnorm(df_keep_combined$perf) + 
    (sensitivity_df$bias_weight[jj]*df_keep_combined$bias)
...
sensitivity_df |> pull(size) |> table() 

ggsave(plot = last_plot(),
       filename = here("plots/SOM2_sensitivity_puzzles.png"),
       height = 10,
       width = 10,
       units = "cm",
       dpi = 400)
...
file.copy(from = paste0(here("data/som_files/som2_files/som2_nclust_"),
                        win_size, "_iter_", win_iter, ".rds"),
          to = here("data/som_files/som_selections/som2_selection_complex.rds"),
          overwrite = TRUE,
          copy.mode = TRUE)
...
ggplot() +
  ...
  geom_point(data = df,     aes(x= som_size, y = bias), col = "grey30", size = 3, alpha = 0.6) +
...
ggsave(plot = last_plot(),
       filename = here("plots/SOM2_element_bias.png"),
       height = 10,
       width = 18,
       units = "cm",
       dpi = 400)
