...
sensitivity_df |> pull(size) |> modal()
...
ggplot() +
  ...
  geom_point(data = df,     aes(x= som_size, y = k_l_scaled), col = "grey30", size = 3, alpha = 0.6) +
...
ggsave(plot = last_plot(),
       filename = here("plots/SOM2_element_k_l_scaled.png"),
       height = 10,
       width = 18,
       units = "cm",
       dpi = 400)
